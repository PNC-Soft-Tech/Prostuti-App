...
class ContestDetailsWidget2 extends GetWidget<ContestDetailsController> {
  const ContestDetailsWidget2({super.key});
            ...
            SizedBox(
              ...
              child: Column(
                children: [
                  ...
                  _buildLeftColumnRow(title: "তারিখ", img: 'assets/contest-start.png'),
                ],
              ),
            ),
            Expanded(
              child: Column(
                ...
                children: [
                  _buildRightColumnRow(
                    value: "${controller.contestDetails.value?.contest.stringTopics}",
                  ),
                  _buildRightColumnRow(
                    value: "${controller.contestDetails.value?.contest.totalMarks} মার্কস",
                  ),
                  _buildRightColumnRow(
                    value: "${controller.contestDetails.value?.contest.totalTime} মিনিট",
                  ),
                  _buildRightColumnRow(
                    value: "${controller.contestDetails.value?.contest.registeredCount} জন",
                  ),
                  _buildRightColumnRow(
                    value: Utils.formatDateToBangla(
                      controller.contestDetails.value?.contest.endContest ?? DateTime.now(),
                    ),
                  ),
                ],
              ),
